...
class ClientNode(Node):
    def __init__(self, robot_name = ''):
        ...
        for i in range(len(self.transmit_topics)):
            self.transmit_objects.append(BridgeObject(self.DIRECTION_TRANSMIT, self.transmit_topics[i], self.transmit_msg_types[i], self.transmit_ports[i], self.transmit_protocols[i], self.transmit_qos[i]))

        for j in range(len(self.receive_topics)):
            self.receive_objects.append(BridgeObject(self.DIRECTION_RECEIVE, self.receive_topics[j], self.receive_msg_types[j], self.receive_ports[j], self.receive_protocols[j], self.receive_qos[j]))
...
def main(argv=sys.argv[1:]):
    ...
    client_node = ClientNode(args.robot_name)
